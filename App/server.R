...
server <- function(input, output, session) {
  ...
  output$publishedprimerTable <- DT::renderDataTable ({
    primers2 <- read_excel("publishedprimers_10May.xlsx") # published primers
   ...
   msa1plot <- eventReactive(input$msa1, {
    if (input$publishedprimerTable_rows_selected) {
      ...
      primers <- read_excel("publishedprimers_10May.xlsx")
  ...
  output$publishedprimerplot <- renderPlot({
    ...
    validate(need(input$publishedprimerTable_rows_selected[1], "Select a primer from the table."))
    ...
    if (primers[s, 7] != "NA") {
      ...
      mutnexts = merge(allp, data.frame(fromfw_torv), by.x = "position", by.y = "fromfw_torv") # to find degenerate nt
      
      
      # isolate count is added to column 3, it will be used to scale color on plot
      al[,3] <-0
      MutationDataMay12 <- data.frame(MutationDataMay12)
      al[which(al[,1]%in%MutationDataMay12[, 1]), 3] = MutationDataMay12[which(MutationDataMay12[, 1]%in%al[,1]), 16]
      
      # V4 column of al contains ""(mut count=0), 0(mut count>=955),1(mut count<955)
      al[which(al[,3]>0),4] <- 2
      al[which(al[,3]<955),4] <- 1 # thresholdun altında kalanları 1 yap
      al[which(al[,3]==0),4] <-3
      ...
      ggplot2::ggplot(al, aes(x = fromfw_torv, y = y)) +
        geom_segment(aes(x = fromfw_torv,xend = fromfw_torv,y = 0, yend = y, linetype="dotted")) + scale_linetype_identity() +
        geom_point(aes(color = (factor(V4)))) +
        scale_color_manual(values = c("orange","red","white")) +
      
        annotate("text",label = rn,x = ((fendpos+1):(rstartpos-1)) + 0.05,y = -0.001,size = 4,colour = "darkgreen" ,fontface =2) +
        annotate("text",label = refforw,x = (fstartpos:fendpos) + 0.05,y = -0.001,size = 4,colour = "blue" ,fontface =2) +
        annotate("text",label = refreverse,x = (rstartpos:rendpos) + 0.05,y = -0.001,size = 4,colour = "blue",fontface =2 ) +
        
        geom_text(
          data = subset(al, al[, 3] > 0),
          aes(label = as.character(mutnexts[, 4])),
          hjust = 0,
          vjust = 0)+ 
        coord_cartesian(xlim = ranges$a1x,
                        ylim = ranges$a1y,
                        expand = FALSE) +
        xlim(fstartpos-0.2, rendpos +0.2)+
        ylim(-0.2, (max(allpos[fromfw_torv, 2])+1.2))+
        xlab("Position") +
        ylab("Percentages of Mutations")+ggtitle(titleofgraph) +
        theme_light() +
  ...
  output$click_info <- DT::renderDataTable({
    ...
    MutationDataMay12 <- data.frame(MutationDataMay12)
   # m<- data.frame(MutationDataMay12[is.element(MutationDataMay12[,1], n), ] )
    # m<- data.frame(MutationDataMay12[MutationDataMay12[,1] %in% n, ] )
    
    #print(m)
    m <- data.frame(MutationDataMay12  %>% filter(position%in% n))
    DT::datatable(data.frame(m),
                  options = list(paging = TRUE,searching =TRUE,lengthMenu = c(5,10, 30, 50),
                                                                               pageLength = 5,
                                                                               scrollX = TRUE))
  ...
  output$publishedphylop <- renderPlot({
    ...
    validate(
      need(input$publishedprimerTable_rows_selected[1], "Select a primer from the table.")
    )
    # plot for amplicon showing mutation places
    primers <- read_excel("publishedprimers_10May.xlsx")
    ...
    if (primers[s, 7] != "NA") {
      ...
      ggplot(data=new, aes(x=pos, y=p)) + geom_segment(aes(x = pos,xend = pos,y = 0,yend = p)) +
        geom_bar(stat="identity", width=1,fill="steelblue")+ ylim((min(new[,2])-1), (max(new[, 2])+1))+
        geom_text(aes(label=p), vjust=1.6, color="black", size=4, angle=15)+ coord_cartesian(xlim = ranges$a1x,
                                                                                             ylim = ranges$a1y,
                                                                                             expand = FALSE) +
        xlab("Position") +
        ylab("PhyloP values")+
  ...
  Conservedprimers <- eventReactive (input$do2, {
    # new primer table
    Conservedprimers = read_csv("Conservedprimers_29May.csv")
  ...
  output$conserverdprimerTable <- DT::renderDataTable ({
    validate(need(input$do2, "Press the 'visualize' button."))
    ...
    datatable(
      ...
      rownames = FALSE,
      escape = FALSE,
      caption = 'Table: Potential conserved primer pairs in your gene of interest',
      filter = "top",
      selection = "single",
  ...
  output$conservedplot <- renderPlot({
    ...
    validate(need(input$conserverdprimerTable_rows_selected[1], "Select a primer from the table."))
    ...
    ggplot(al, aes(x = fromfw_torv, y = y)) +
      geom_segment(aes(x = fromfw_torv,xend = fromfw_torv,y = 0,yend = y,linetype="dotted")) +
      ...
      geom_text(
        ...
        hjust = 0,
        vjust = 0) +
      coord_cartesian(xlim = ranges$cx, ylim = ranges$cy,expand = FALSE) +
      xlab("Position") +
      ylab("Percentages of Mutations")+
      ggtitle("Visualizing Primer Binding Region and Amplicon") +
      xlim(fstartpos-0.2, rendpos +0.2)+
      ylim(-0.2, max(allpos[fromfw_torv, 2])+0.4 )+
      theme_light() +
      theme(plot.title = element_text(color="red", size=14, face="bold",hjust=0.5),
            legend.position = "none",
            panel.grid.major.x = element_blank(),
  ...
  output$conservedphylop <- renderPlot({
    ...
    ggplot(data=new, aes(x=pos, y=p)) + geom_segment(aes(x = pos,xend = pos,y = 0,yend = p)) +
      geom_bar(stat="identity", width=1,fill="steelblue")+ ylim((min(new[,2])-1), (max(new[, 2])+1))+
      geom_text(aes(label=p), vjust=1.6, color="black", size=4, angle=15)+ coord_cartesian(xlim = ranges$cx,
                                                                                           ylim = ranges$cy,
                                                                                           expand = FALSE) +
      xlab("Position") +
      ylab("PhyloP values")+
  ...
  output$mutationinformation_cons <- renderDataTable({
    ...
    m <- data.frame(MutationDataMay12  %>% filter(position%in% fromfw_torv))
    DT::datatable(data.frame(m), 
                  options = list(paging = TRUE,searching =TRUE,lengthMenu = c(5,10, 30, 50),
                                                                                     pageLength = 5,
                                                                                     scrollX = TRUE))
  ...
  # degenerate primer
  Degeneratedprimers <- eventReactive (input$do4, {
    
    # new primer table
    
    degeneratedprimers <-data.frame(read_csv("Degenerateprimers_12May.csv"))
  ...
  output$degenerateprimerTable <- DT::renderDataTable ({
    ...
    validate( need(input$do4, "Press the 'visualize button.'"))
    ...
    datatable(
      ...
      rownames = FALSE,
      escape = FALSE,
      caption = 'Table: Degenerate primer pairs in your gene of interest.',
      filter = "top",
      selection = "single",
      options = list(initComplete = JS(
        "function(settings, json) {",
  ...
  output$degenerateplot <- renderPlot({
    validate(need(input$degenerateprimerTable_rows_selected[1], "Select a primer from the table."))
    ...
    ggplot(al, aes(x = fromfw_torv, y = y)) +
      ...
      geom_text(
        ...
        hjust = 0,
        vjust = 0) +
      coord_cartesian(xlim = ranges$dx, ylim = ranges$dy,expand = FALSE) +
      xlab("Position") +
      ylab("Percentages of Mutations")+
      ggtitle("Visualizing Primer Binding Region and Amplicon") +
      xlim(fstartpos-0.2, rendpos +0.2)+
  ...
  output$degeneratephylop <- renderPlot({
    ...
    ggplot(data=new, aes(x=pos, y=p)) + geom_segment(aes(x = pos,xend = pos,y = 0,yend = p)) +
      geom_bar(stat="identity", width=1,fill="steelblue")+ ylim((min(new[,2])-1), (max(new[, 2])+1))+
      geom_text(aes(label=p), vjust=1.6, color="black", size=4, angle=15)+ coord_cartesian(xlim = ranges$dx,
                                                                                           ylim = ranges$dy,
                                                                                           expand = FALSE) +
      xlab("Position") +
      ylab("PhyloP values")+
  ...
  output$mutationinformation_deg <- renderDataTable({
    ...
    m <- data.frame(MutationDataMay12  %>% filter(position%in% fromfw_torv))
    DT::datatable(data.frame(m), options = list(paging = TRUE,searching =TRUE,lengthMenu = c(5,10, 30, 50),
                                                                                      pageLength = 5,
                                                                                      scrollX = TRUE))
  ...
  forward <- reactive({
    ...
    validate(need( grepl(Pattern, input$forwardtext)==FALSE, "Remove these letters from your sequence: E, F, I, J, L, O, P, Q, U, V, X, Z"))
    validate(need( grepl(Pattern1, input$forwardtext)==FALSE, "Remove these letters from your sequence: E, F, I, J, L, O, P, Q, U, V, X, Z"))
    validate(need( nchar(input$forwardtext)>10, "Enter longer sequence."))
  ...
  reversepr <- reactive({
    input$do3
    validate(need( nchar(input$reversetext)>10, "Enter longer sequence."))
    codes=c("A","C","G","T","M","R","W","S","Y","K"," V","H","D","B","N")
    noncodes=LETTERS[!LETTERS %in% codes]
    Pattern = paste(noncodes, collapse="|")
    Pattern1 = paste(tolower(noncodes), collapse="|")
    ...
    validate(need( grepl(Pattern, input$reversetext)==FALSE, "Remove these letters from your sequence: E, F, I, J, L, O, P, Q, U, V, X, Z"))
    validate(need( grepl(Pattern1, input$reversetext)==FALSE, "Remove these letters from your sequence: E, F, I, J, L, O, P, Q, U, V, X, Z"))
    
    validate(need(input$reversetext != input$forwardtext, "Please enter different forward or reverse pair."))
  ...
  output$searchyourprimersTable <- DT::renderDataTable ({
    ...
    validate(need(input$forwardtext, "Enter a valid sequence for your forward primer."))
    
    validate(need(input$reversetext, "Enter a valid sequence for your reverse primer."))
    validate(need( input$do3, "Press Search button."))
  ...
  output$searchplot <- renderPlot({
    validate(need(input$forwardtext, "Enter a valid sequence for your primers.") )
    
    validate(need(input$reversetext, "Enter a valid sequence for your primers."))
    ...
    if (input$align){
      ...
      ggplot(al, aes(x = fromfw_torv, y = y)) +
        ...
        geom_text(
          ...
          hjust = 0,
          vjust = 0) +
        coord_cartesian(xlim = ranges$a2x, ylim = ranges$a2y,expand = FALSE) +
        xlab("Position") +
        ylab("Percentages of Mutations")+
        ggtitle("Visualizing Primer Binding Region and Amplicon") +
        xlim(fstartpos-0.2, rendpos +0.2)+
  ...
  output$searchphylop<- renderPlot({
    ...
    if (input$align){
      ...
      ggplot(data=new, aes(x=pos, y=p)) + geom_segment(aes(x = pos,xend = pos,y = 0,yend = p)) +
        geom_bar(stat="identity", width=1,fill="steelblue")+ylim((min(new[,2])-1), (max(new[, 2])+1))+
        geom_text(aes(label=p), vjust=1.6, color="black", size=4,angle=15)+ coord_cartesian(xlim = ranges$a2x,
                                                                                            ylim = ranges$a2y,
                                                                                            expand = FALSE) +
        xlab("Position") +
        ylab("PhyloP values")+
  ...
  output$mutationinformation_search <- renderDataTable({
  ...
  output$conserverdprimerTabledownload = downloadHandler(
    filename = "selected_conserved_primers.csv",
    content = function(file) {
      write.csv(Conservedprimers(), file)
    }
  )
  # degenerate table
  output$degenerateprimerTabledownload = downloadHandler(
    filename = "selected_degenerate_primers.csv",
    content = function(file) {
      write.csv(Degeneratedprimers(), file)
    }
  )
  ...
  #MutationDataMay12 <- read_csv("MutationDataMay12.csv", col_types = cols(X1 = col_skip()))
  MutationDataMay12 <- read.csv("MutationDataMay12.csv",header = T)
  
  MutationDataMay12 <-  MutationDataMay12 %>% mutate_if(is.numeric, ~round(., 3))
  
  MutationDataMay12 <- MutationDataMay12[,-1]
  colnames(  MutationDataMay12)[1] <- "position"
  ...
  #  only positions >0.1
  nexts <-  data.frame(read_csv("May12_0.1threshold-iupac-change-percent.csv"))
