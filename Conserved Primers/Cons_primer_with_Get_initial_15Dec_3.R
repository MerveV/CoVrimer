# downloaded data, total sequence is 6854423
...
for (r in 1:dim(t)[1]) {
  ...
  forw1 <- forw1%>% rowwise() %>% 
    dplyr::mutate(fwtm=specify_decimal(as.numeric(calculate_tm(fw)),2),
           fwgc=specify_decimal(as.numeric(seqinr::GC(s2c(fw)))*100,2)) 
  ...
  rev1 <- rev1%>% rowwise() %>% 
    dplyr::mutate(rvtm=specify_decimal(as.numeric(calculate_tm(rv)),2),
           rvgc=specify_decimal(as.numeric(seqinr::GC(s2c(rv)))*100,2)) 
...
dim(final) #  596606  -> 582161  <- 600293  -> 614183
...
final1 <- final %>%
  ...
  dplyr::filter(between(amplicon,60,180))

dim(final1) # 37329 ->   34050   42281  43589
# final2 <- final1%>%
#   filter(fwlength <= 25) %>%
#   filter(revlength <= 25)
...
dim(final1run) #36673 -> 33413  41644
...
dim(final1runclampt) #13151    9783
...
forreverse <- sss %>%dplyr::group_by(`Reverse Primer`)%>%
...
forreverse <- forreverse %>%dplyr::group_by(`Rv End`)%>%
  dplyr::slice(which.max(`Fw SARS-CoV-2 Specificity`)) %>%
  ungroup()
forfor <- sss %>%dplyr::group_by(`Forward Primer`)%>%
...
write.csv(finalpairs,"Conservedprimers_15Dec.csv",row.names = F)
